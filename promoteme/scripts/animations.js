      ...
      (entries, obs) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const staggerables = entry.target.querySelectorAll(
              ".proj-card, .srv-card, .skill-chip, .badge, .contact-link",
            );
            staggerables.forEach((el, i) => {
              ...
              if (!el.style.transition)
                el.style.transition =
                  "opacity 520ms cubic-bezier(.2,.9,.2,1), transform 520ms cubic-bezier(.2,.9,.2,1)";
